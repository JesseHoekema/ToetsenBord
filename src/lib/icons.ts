// icons.ts
const iconModules = import.meta.glob("./som-icons/*.icon.ts", { eager: true });

const iconMap: Record<string, { name: string; data: string }> = {};

for (const path in iconModules) {
  const module = iconModules[path] as any;
  const iconExport = Object.values(module)[0] as { name: string; data: string };
  iconMap[iconExport.name.toLowerCase()] = iconExport;
}

function normalize(vak: string) {
  return vak
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
}

export function getIcon(vak: string): string {
  const key = normalize(vak);

  // Exact match
  const exactMatch = Object.keys(iconMap).find((k) => normalize(k) === key);
  if (exactMatch) return iconMap[exactMatch].data;

  // Fuzzy match
  const fuzzyMatch = Object.keys(iconMap).find(
    (k) => normalize(k).includes(key) || key.includes(normalize(k)),
  );
  if (fuzzyMatch) return iconMap[fuzzyMatch].data;

  // Always return this SVG if not found
  return '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 20 20"><path fill="#3E721D" d="M19.444 14.444a2.22 2.22 0 0 1-2.222 2.223H2.778a2.22 2.22 0 0 1-2.222-2.223V3.507C.556 2.28 3.773 0 5 0h11.458c1.72 0 2.986 1.284 2.986 2.986z"/><path fill="#CCD6DD" d="M18.333 16.667a2.22 2.22 0 0 1-2.222 2.222H3.89a2.22 2.22 0 0 1-2.222-2.222V3.333c0-2.288-.012-2.222 2.777-2.222h11.667c1.227 0 2.222.995 2.222 2.222z"/><path fill="#E1E8ED" d="M17.222 17.222c0 .92-.746 1.667-1.667 1.667H2.223c-.92 0-1.666-.746-1.666-1.667V3.89c0-.92.746-1.667 1.666-1.667h13.334c.92 0 1.666.746 1.666 1.667z"/><path fill="#5C913B" d="M17.222 17.778A2.22 2.22 0 0 1 15 20H3.333a2.22 2.22 0 0 1-2.222-2.222V5.556c0-1.228.995-2.223 2.222-2.223H15c1.227 0 2.222.995 2.222 2.223z"/><path fill="#77B255" d="M16.111 17.778A2.22 2.22 0 0 1 13.89 20H3.333a2.22 2.22 0 0 1-2.222-2.222V6.667c0-1.228.995-2.223 2.222-2.223h10.742c1.227 0 2.036.81 2.036 2.037z"/><path fill="#3E721D" d="M3.333 3.333c-.937 0-.961-1.067-.555-1.527.462-.523 1.18-.695 2.465-.695h.868V0h-1.51C2.396 0 .556 1.389.556 2.986v14.792C.556 19.005 1.55 20 2.778 20h1.11V3.333z"/></svg>';
}
