  ...
  try {
    ...
    if (tokenParts.length !== 3) {
      return {
        success: false,
        message:
          "Ongeldig token formaat. Verwacht formaat: access_token!expires_at!refresh_token",
      };
    }
  ...
  try {
    ...
    if (!decryptedRefreshToken) {
      return {
        success: false,
        message:
          "Token kon niet worden ontsleuteld. Verbind opnieuw met Somtoday.",
      };
    }
...
export async function getExam(vak: string, datum: string, authToken: string) {
  ...
  const matchingExam = exams.find((exam: any) => {
    ...
    return (
      examVak === normalizedVak && month === targetMonth && day === targetDay
    );
  });
...
export async function getAllExams(authToken: string) {
  ...
  const oncomingExams = await getExams(authToken);
  ...
  const resolvedOncomingExams = oncomingExams;

  if (oncomingExams) {
    oncomingExams.forEach((oncomingExam: any) => {
      const exists = exams.some(
        (exam: any) =>
          exam.onderwerp === oncomingExam.onderwerp &&
          exam.vak === oncomingExam.vak &&
          exam.datum === oncomingExam.datum,
      );

      if (!exists) {
        exams.push(oncomingExam);
      }
    });
  }
...
export async function getGrades(authToken: string) {
  try {
    ...
    const response = await fetch(
      `https://api.somtoday.nl/rest/v1/geldendvoortgangsdossierresultaten/leerling/${id}?type=Toetskolom&type=DeeltoetsKolom&type=Werkstukcijferkolom&type=Advieskolom&additional=vaknaam&additional=resultaatkolom&additional=naamalternatiefniveau&additional=vakuuid&additional=lichtinguuid`,
      {
        headers: {
          Authorization: `Bearer ${decryptedToken}`,
          Accept: "application/json",
    ...
    const grades = data.items
      ...
      .map((item: any) => {
        const vaknaam = item.additionalObjects?.vaknaam
          ? item.additionalObjects.vaknaam
          : item.vak?.naam || "Onbekend";

        const isCijfer = item.isCijfer === true;
        const cijfer =
          isCijfer && typeof item.cijfer !== "undefined"
            ? parseFloat(String(item.cijfer).replace(",", "."))
            : null;
        const label =
          !isCijfer && typeof item.label !== "undefined" ? item.label : null;

        const grade = isCijfer
          ? item.formattedResultaat || item.cijfer || item.resultaat
          : item.label || item.formattedResultaat;

        const rawDate =
          item.datumInvoerEerstePoging || item.datumInvoer || null;
        const datum = rawDate
          ? new Date(rawDate).toLocaleDateString("nl-NL", {
              day: "numeric",
              month: "short",
            })
          : null;
...
export async function getHomework(authToken: string) {
  ...
  const homework = await Promise.all(
    data.items
      .filter((item: any) =>
        item.studiewijzerItem?.huiswerkType
          ?.toLowerCase()
          ?.includes("huiswerk"),
      )
      .map(async (item: any) => {
        ...
        const homeworkLink = `/homework/${vak
          ...
          .replace(
            /^-+|-+$/g,
            "",
          )}-${item.datumTijd ? new Date(item.datumTijd).toISOString().split("T")[0] : "no-date"}`;
        ...
        return {
          ...
          vak,
          datum: item.datumTijd
            ? new Date(item.datumTijd).toLocaleDateString("nl-NL", {
                day: "numeric",
                month: "long",
              })
            : null,
    ...
    data.items
      ...
      .map(async (item: any) => {
        ...
        return {
          ...
          imageUrl,
          link: homeworkLink,
          bijlagen: (item.studiewijzerItem?.bijlagen || []).flatMap(
            (bijlage: any) =>
...
export async function getHomeworkItem(
  vak: string,
  datum: string,
  authToken: string,
) {
