...
<svelte:head>
    <title>Toetsen | ToetsenBord</title>
</svelte:head>
...
<Sidebar.Provider>
    ...
    <Sidebar.Inset>
        ...
        <main class="flex-1 overflow-y-auto p-6 pt-2">
            <div class="">
                <h2 class="text-2xl font-semibold mb-2">Alle Toetsen</h2>
                <div class="flex gap-8 flex-wrap">
                    {#each (data.exams ?? []).slice().reverse() as exam}
                        <a href={exam.link}>
                            <Card.Root
                    ...
                    {#if !data.exams || data.exams.length === 0}
                        <div class="flex flex-col gap-4 mt-3">
                            <h1 class="text-lg">Om toetsen te laten zien koppel eerst somtoday!</h1>
